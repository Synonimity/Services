...
class Customer(Base):
    """
    Deliberately thin and decoupled from any particular auth system.
    `external_user_id` is whatever your app's own user ID is - this
    service doesn't care if that's a UUID, an int, or a string from a
...
class Plan(Base):
    """A billing plan you define and seed yourself - not synced from PayFast."""
    ...
    amount: Mapped[float] = mapped_column(Numeric(10, 2), nullable=False)  # ZAR
    # PayFast frequency codes: 3=monthly, 4=quarterly, 5=biannual, 6=annual
    frequency: Mapped[int] = mapped_column(SmallInteger, nullable=False, default=3)
    cycles: Mapped[int] = mapped_column(SmallInteger, nullable=False, default=0)  # 0 = indefinite
...
class Subscription(Base):
    ...
    # Our own reference, generated before the customer ever reaches PayFast,
    # used to reconcile the ITN callback to this row.
    m_payment_id: Mapped[str] = mapped_column(String(100), unique=True, nullable=False)

    # PayFast's subscription token - only known once the first payment's
    # ITN arrives. This is what the Subscriptions API (pause/cancel/etc) is
    # keyed on.
...
class Payment(Base):
    """Every ITN we've received, kept as an audit trail. Append-only."""
